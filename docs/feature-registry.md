...
| Parallel cluster decompression | ✅ Complete | Workqueue-based (minimem_dec); atomic completion; 32-page clusters; **2.7× speedup on 4 vCPUs** (8GB QEMU, custom kernel); rwsem protects map entries from concurrent shrinker/drain; NULL return checks for zs_obj_read_begin; auto-detect mode (disabled on 1-CPU, auto on ≥2, forced on/off via sysfs) |
...
| Sysfs stats interface | ✅ Complete | 38 attributes: compress/decompress counts, latencies, zswap pages/bytes/saved, pool_pages, max_pool_pages, benchmark timings, scanner knobs, hook_faults, kernel_patches, parallel stats, scanner_pages_compressed/skipped, scanner_cycles_total/cycles_empty, scanner_current_interval_ms, parallel_mode, zap_cb_count, zap_cb_miss_count, mmu_release_count, mmu_release_pages, compress_pid, scanner_cpu_budget_ms, scanner_mark_budget_pages, scanner_mark_yielded, scanner_sweep_yielded |
| Memory overhead tracking | ✅ Complete | pool_pages sysfs attribute; zswap_pages/bytes/saved for compression efficiency |
| Concurrency (RCU-safe, per-CPU) | ✅ Complete | RCU-safe xarray lookups; spinlock-protected writes; per-CPU compress/decompress buffers; rw_semaphore protects map entries from concurrent removal during parallel decompress (shrinker/drain take write lock) |
...
| QEMU VM test infrastructure | ✅ Complete | research/024; vm-test-minimem.sh with 7-phase progressive testing; kselftest (65 tests), E2E, transparent compression, kprobe, scanner roundtrip, CPU overhead, drain-and-restore, stress tests; kretprobe enables scanner sweep on stock kernels |
...
| Performance test harness | ✅ Complete | test_perf_harness.c: 6-test suite — latency (p50=0.1μs, p95=0.2μs, p99=0.4μs on QEMU/KVM), throughput, concurrency stress (10.2M ops/sec, 8 threads), activity pattern, memory overhead (100% savings, 32 bytes/page), scanner re-compression; CSV output; --quick/--pages/--threads/--rounds/--csv options; 8GB VM test preset |
...
| Scanner sweep (kretprobe fallback) | ✅ Complete | kretprobe on do_swap_page with entry+return handlers; entry handler decompresses page and installs present PTE; return handler changes VM_FAULT_SIGBUS to VM_FAULT_NOPAGE for MiniMem faults; p99 decompress latency 0.4µs (stock) vs 0.3µs (custom); MMU notifier cleanup on module unload releases all notifiers (users count) to prevent dangling ops pointers; see research/025 |
| Per-process compression (sysfs) | ✅ Complete | `compress_pid` sysfs attribute: write PID to compress all eligible anonymous pages for that process; uses `minimem_compress_and_replace_pte()` + `mmap_read_trylock`; registers MMU notifier for cleanup |
...
| Scanner performance safeguards | ✅ Complete | CPU budget per phase (scanner_cpu_budget_ms, default 50ms, range 1-1000); mark batch limit (scanner_mark_budget_pages, default 65536, range 4096-524288); cursor-based resumption (pid+addr) for both mark and sweep phases; cond_resched() every 256 pages (mark) / 64 pages (sweep); sweep batch limit 8192 pages/cycle; adaptive interval backs off 2s/cycle (max 30s), resets on compression |
...
| Page drain-and-restore on unload | ✅ Complete | minimem_zswap_drain_and_restore() walks all processes' page tables, decompresses every MiniMem PTE marker, restores PTEs to present before module unload; no data loss; zap_pte_range CONFIG_MINIMEM branch catches PTE markers on process exit/munmap and calls minimem_zap_cb to free zsmalloc |
| MMU notifier (process exit cleanup) | ✅ Complete | minimem_mmu.c/h; registers per-mm MMU notifier via mmu_notifier_get(); release callback walks page tables on process exit, clears MiniMem PTE markers, frees zsmalloc; eliminates "unrecognized swap entry" warnings on stock kernels; deferred registration via workqueue (mmu_notifier_get takes mmap_write_lock internally); mmap_read_trylock in scanner/drain avoids deadlock; pte_clear prevents kernel from seeing stale swap entries; module exit releases all notifiers (with users count) to prevent dangling ops pointer crash |
...
| Scanner skip-reason counters | ✅ Complete | 14 sysfs attributes: scanner_mark_pages, scanner_skip_vma_locked, scanner_skip_page_shared, scanner_skip_page_mlocked, scanner_skip_incompressible, scanner_cycles_total, scanner_cycles_empty, scanner_current_interval_ms, scanner_pages_scanned, scanner_pages_idle, scanner_mark_yielded, scanner_sweep_yielded, scanner_cpu_budget_ms, scanner_mark_budget_pages |
