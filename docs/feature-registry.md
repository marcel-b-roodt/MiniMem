...
| Page drain-and-restore on unload | ✅ Complete | minimem_zswap_drain_and_restore() walks all processes' page tables, decompresses every MiniMem PTE marker, restores PTEs to present before module unload; no data loss; ⚠️ "unrecognized swap entry" flood on module unload when PTE markers remain in exited process page tables (minimem_zap_cb not catching all entries — separate bug) |
